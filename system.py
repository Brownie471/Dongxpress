...
def login():
    if request.method == 'GET':
        ...
        if (len(name) == 0 and len(password) == 0) or len(name) == 0:
            return render_template('login_fail_noentry.html')
        elif len(password) == 0:
            return render_template('login_fail_noentry_nopw.html', name=name)
...
SELECT * FROM Customer_Account        
        ...
        if len(result) == 0:
            db.close()
            return render_template('login_fail_wrongpw.html', name = name)
        else:
            result = result[0]
            query_login = '''
            ...
            db.commit()
            db.close()
            return render_template('account_home.html', name = result[0], class_ = result[1], indexno = result[2])

#password change#-----------------------------------------------------------------------------------------------
@app.route('/changepw/', methods=['GET','POST'])
def changepw():
    if request.method == 'GET':
        return render_template('account_changepw.html')
    else:
        ...
        if curr_pw != correct_pw:
            db.close()
            return render_template('account_changepw_failwrongpw.html')
        else:
            ...
            db.commit()
            db.close()
            return render_template('account_changepw_success.html')
...
def register():
    if request.method == 'GET':
        ...
        if pw1 != pw2:
            return render_template('register_failpw.html', name = name, class_ = class_, index_no = index_no)
        elif len(name) == 0 or len(class_) == 0 or len(index_no) == 0 or len(pw1) == 0:
            return render_template('register_failempty.html', name = name, class_ = class_, index_no = index_no)
            ...
            if len(cursor.fetchall()) > 0:
                db.close()
                return render_template('register_failexist.html')
...
def owner_login():
    if request.method =='GET':
        return render_template('login_owner.html', incorrect = '')
    else:
        p1 = request.form['pw']
        path = os.path.dirname(os.path.abspath(__file__))
        file = os.path.join(path, 'library.db')
...
ORDER BY Delivery_Date ASC
            ...
            return render_template('result_owner.html', result_250 = result_250, result_cust = ultra_final)

        else:
            return render_template('login_owner.html', incorrect = '*The entered password is incorrect*')
...
def account_home():
    ...
    db.close()
    return render_template('account_home.html', name=name)
...
def order250():
    if request.method == 'GET':
        ...
        db.close()
        #print(cai)
        return render_template('order_250.html', cai = cai, not_cai = not_cai)
    else:
        ...
        if day < 29:
            ...
            print(actual)
            return render_template('order_comfirm250.html', result = result, date_ = actual)
        elif month != 12:
            month += 1
            day = 1
            actual = str(year) + '-' + str(month)+ '-' + str(day)
...
def thank_250():
    ...
    try:
        ...
        try:
            ...
            db.close()
            return render_template('order_too_many.html', type_ = 'custom order')

    except sqlite3.IntegrityError:
        

        db.close()
        return render_template('order_too_many.html', type_ = '$2.50 meal')
...
def order_cust():
    if request.method == 'GET':
        ...
        return render_template('order_cust.html', cai=all_cai, rou=all_rou)
    else:
        query_get_user = '''
        SELECT Logined.Name, Logined.Class, Logined.IndexNo
        FROM Logined
...
def shopping_cart():
    ...
    if day < 29:
        ...
        print(actual)
        return render_template('order_cust_shoppingcart.html', result = result, date_ = actual,total = total)
    elif month != 12:
        month += 1
        day = 1
        actual = str(year) + '-' + str(month)+ '-' + str(day)
        ...
        actual = str(year) + '-' + str(month)+ '-' + str(day)
        print(actual)
    return render_template('order_cust_shoppingcart.html', result = result, total = total, date_ = actual)
...
def add_shopping_cart_c():
    ...
    return render_template('order_cust_success.html', cai=all_cai, rou=all_rou)
#-----------------------------------------------------------------------------------------------
...
def gateway_order():
    return render_template('gateway_order.html')
